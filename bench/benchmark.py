...
Simulates the same three regeneration tasks against examples/sales, twice:
  ...
  heddle mode — the get_contract packet, a blast-radius check, and the verify
...
def raw_task_tokens(store: Store, name: str) -> int:
    ...
    # the suite itself (heddle runs -q server-side and serves a summary instead)
...
def heddle_task_tokens(store: Store, name: str) -> int:
...
def main() -> None:
    ...
    heddle = {t: heddle_task_tokens(store, t) for t in TASKS}
    cache = api.status(SALES, store)["cache"]

    width = max(map(len, TASKS))
    print(f"\n{'task':<{width}}  {'raw files':>10}  {'heddle':>10}  {'reduction':>10}")
    print("-" * (width + 38))
    for t in TASKS:
        print(f"{t:<{width}}  {raw[t]:>10,}  {heddle[t]:>10,}  {raw[t] / heddle[t]:>9.1f}x")
    total_raw, total_heddle = sum(raw.values()), sum(heddle.values())
    print("-" * (width + 38))
    print(f"{'total':<{width}}  {total_raw:>10,}  {total_heddle:>10,}  {total_raw / total_heddle:>9.1f}x")
    print(f"\nverification cache during heddle run: {cache['hits']} hits / {cache['misses']} misses\n")

    if total_raw / total_heddle < 5:
